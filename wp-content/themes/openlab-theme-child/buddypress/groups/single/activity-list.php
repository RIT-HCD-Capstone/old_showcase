<!-- yanked from openlab_group_profile_activity_list() of openlab-theme/lib/group-funcs.php -->
<!-- the purpose of this file is to bring the following code into the template directory, so it can be edited -->
		...
		<?php if ( bp_is_group_home() ) { ?>
			...
			<?php if ( 'public' === bp_get_group_status() || ( ( 'hidden' === bp_get_group_status() || 'private' === bp_get_group_status() ) && ( bp_is_item_admin() || bp_group_is_member() ) ) ) : ?>
				...
				if ( cboxol_site_can_be_viewed() ) {
					//openlab_show_site_posts_and_comments(); /* prevents loading the 'Recent Posts' and 'Recent Comments' menus */
				}
				?>
					...
					<div class="row group-activity-overview">
						...
						<?php if ( function_exists( 'bp_docs_get_slug' ) && openlab_is_docs_enabled_for_group( $group->id ) ) : ?>
							<div class="col-sm-12">
								<div id="recent-docs">
									<div class="recent-posts">
										<h2 class="title activity-title"><a class="no-deco" href="<?php echo esc_attr( $group_url ); ?><?php echo esc_attr( bp_docs_get_slug() ); ?>/"><?php esc_html_e( 'Recent Posts'/*'Recent Docs'*/, 'commons-in-a-box' ); ?><span class="fa fa-chevron-circle-right" aria-hidden="true"></span></a></h2>
										<?php
